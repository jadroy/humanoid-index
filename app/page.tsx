...
const SWITCHER_STYLES = ["drag", "single", "toggle", "pill", "slash", "dot", "dash", "brackets", "ghost", "divider"] as const;
type SwitcherStyle = (typeof SWITCHER_STYLES)[number];

// ─── Drag Switcher ─────────────────────────────────────────────
function DragSwitcher({ active, onChange }: { active: Layout; onChange: (l: Layout) => void }) {
  const containerRef = useRef<HTMLDivElement>(null);
  const [isDragging, setIsDragging] = useState(false);
  const [dragLeft, setDragLeft] = useState<number | null>(null);
  const dragRef = useRef<{ pointerX: number; startLocalX: number; thumbStart: number; thumbW: number; moved: boolean } | null>(null);

  const activeIdx = ALL_LAYOUTS.indexOf(active);
  const isRight = activeIdx === 1;
  ...
  const onPointerDown = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    const restingLeft = 2 + activeIdx * thumbW;
    dragRef.current = {
      pointerX: e.clientX,
      startLocalX: e.clientX - rect.left,
      thumbStart: restingLeft,
      thumbW,
      moved: false,
    };
    setIsDragging(true);
    setDragLeft(restingLeft);
  };
  ...
  const onPointerMove = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    const maxLeft = 2 + dragRef.current.thumbW;
    const next = Math.max(2, Math.min(maxLeft, dragRef.current.thumbStart + dx));
    setDragLeft(next);
  };

  const onPointerUp = () => {
    if (!dragRef.current) return;
    const { moved, thumbW, startLocalX } = dragRef.current;
    const leftNow = dragLeft ?? 0;
    setIsDragging(false);
    setDragLeft(null);

    if (!moved) {
      // Pure click — pick the half that was clicked
      const clickedRight = startLocalX > 2 + thumbW;
      const target = ALL_LAYOUTS[clickedRight ? 1 : 0];
      if (target !== active) onChange(target);
    } else {
      // Drag released — snap to nearest
      const progress = thumbW > 0 ? (leftNow - 2) / thumbW : 0;
      const snap = progress > 0.5 ? ALL_LAYOUTS[1] : ALL_LAYOUTS[0];
      if (snap !== active) onChange(snap);
    }
    dragRef.current = null;
  };

  // Progress 0..1 for color interpolation
  const liveProgress = isDragging && dragLeft !== null && dragRef.current
    ? Math.max(0, Math.min(1, (dragLeft - 2) / dragRef.current.thumbW))
    : (isRight ? 1 : 0);
  const leftInk = `rgba(38, 38, 38, ${1 - liveProgress * 0.65})`;
  const rightInk = `rgba(38, 38, 38, ${0.35 + liveProgress * 0.65})`;

  const thumbStyle: React.CSSProperties = {
    width: `calc((100% - 4px) / 2)`,
    left: isDragging && dragLeft !== null
      ? dragLeft
      : `calc(2px + ${activeIdx} * ((100% - 4px) / 2))`,
    background: "#fff",
    transform: isDragging ? "scale(0.96)" : "scale(1)",
    transition: isDragging
      ? "transform 180ms ease"
      : "left 340ms cubic-bezier(0.22, 1, 0.36, 1), transform 220ms ease",
    pointerEvents: "none",
  };
  ...
  return (
    ...
    >
      <div className="absolute top-0.5 bottom-0.5 rounded-full" style={thumbStyle} />
      {ALL_LAYOUTS.map((l, i) => (
        <div
          key={l}
          className="relative px-3 py-1 text-[11px] tracking-wide"
          style={{
            color: i === 0 ? leftInk : rightInk,
            fontWeight: 500,
            pointerEvents: "none",
            transition: isDragging ? "none" : "color 200ms ease",
          }}
        >
          {layoutLabels[l]}
        </div>
...
function LayoutSwitcher({
  ...
  // ── Style: underline — 2-view switcher variants ──
    ...
    if (switcherStyle === "drag") {
      switcherEl = <DragSwitcher active={active} onChange={onChange} />;
      ...
      switcherEl = (
        <div className="relative flex items-center p-0.5 rounded-full" style={{ background: "rgba(0,0,0,0.04)" }}>
          <div
            className="absolute top-0.5 bottom-0.5 rounded-full"
            style={{
              width: `calc((100% - 4px) / 2)`,
              left: isRight ? `calc(2px + (100% - 4px) / 2)` : "2px",
              background: "#fff",
    ...
    navEl = (
      <nav className="fixed top-0 left-0 right-0 z-50 pointer-events-none" style={{ paddingTop: "var(--nav-top, 4px)", paddingLeft: "var(--arc-logo-x, 24px)", paddingRight: "var(--arc-logo-x, 24px)" }}>
        <div className="flex items-center gap-4">
          <div className="pointer-events-auto">{mark}</div>
          <div className="flex-1 flex justify-center">
            <div className="pointer-events-auto">{switcherEl}</div>
          </div>
          <div style={{ width: 20 }} />
        </div>
      </nav>
...
// ═══════════════════════════════════════════════════════════════
function Browse({ goToIndex, navStyle, onNavStyleChange, switcherStyle, onSwitcherStyleChange, luckyNonce = 0, addHintNonce = 0, onEnterCompare }: { goToIndex?: number | null; navStyle: NavStyle; onNavStyleChange: (s: NavStyle) => void; switcherStyle: SwitcherStyle; onSwitcherStyleChange: (s: SwitcherStyle) => void; luckyNonce?: number; addHintNonce?: number; onEnterCompare?: () => void }) {
  ...
  return (
    ...
    >
      ...
      <div className={introDone ? "intro-content" : "opacity-0"}>
        {layout === "E" && <Browse goToIndex={goToIndex} navStyle={navStyle} onNavStyleChange={setNavStyle} switcherStyle={switcherStyle} onSwitcherStyleChange={setSwitcherStyle} luckyNonce={luckyNonce} addHintNonce={addHintNonce} onEnterCompare={() => setComparingUsed(true)} />}
